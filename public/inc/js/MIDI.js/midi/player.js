...
midi.getFileInstruments = function() {
	var instruments = {};
	var programs = {};
	MIDI.programChange(9, MIDI.GM.byName['synth_drum'].number, 0);
...
var scheduleTracking = function(channel, note, currentTime, offset, message, velocity, time) {
	return setTimeout(function() {
		...
		if (window.App)
		{
			window.App.audio.currentTime = currentTime;
			window.App.audio.duration = midi.endTime;
		}
		///
		eventQueue.shift();
		///
	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		switch (event.subtype) {
			case 'controller':
				...
				break;
			case 'programChange':
				console.log(channelId + " " + event.programNumber + " " + delay);
