...
app_module.controller('main_controller', function($scope) {
    ...
    var generate = $scope.generate = function(seed){
        var song = $scope.currentSong = newMusicSong(seed);
    ...
    function resetHelper(helper)
    {
        ...
        helper.changeSong = function(param) {
            if (param === 'p')
            {
                MIDI.Player.stop();
                MIDI.Player.resume();
    ...
    function newMusicSong(seed){
        ...
        return {name: newMusicName(), data: 'data:audio/midi;base64,' + newMusicData()};
    }
});
