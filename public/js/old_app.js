...
app.controller('playerController', function($scope){
	...
	MIDI.loadPlugin({
		...
		onsuccess : function(){
			console.log('finish');
			MIDI.programChange(1, MIDI.GM.byName["trumpet"].number)			
			MIDI.programChange(2, MIDI.GM.byName["trumpet"].number)			
			MIDI.programChange(3, MIDI.GM.byName["trumpet"].number)			
			MIDI.programChange(4, MIDI.GM.byName["trumpet"].number)			
			MIDI.programChange(5, MIDI.GM.byName["trumpet"].number)			
		    MIDI.Player.loadFile("song/Objection/Pursuit - Cornered.mid", MIDI.Player.start);
		}
	});

	$scope.annalyze = function(){
		// RequestMidi(midiSong, function(data){
		// 	console.log(new NoteSequence(MidiFile(data)).getSequence());			
		// });
		// RequestMidi('http://localhost:8080/song/Neon_Genesis_Evangelion/deployment.mid', function(data){
		// 	console.log(new NoteSequence(MidiFile(data)).getSequence());			
		// });
		// RequestMidi('http://localhost:8080/song/Hans Zimmer - Time.mid', function(data){
		// 	var file = MidiFile(data);
		// 	console.log(file);
		// 	console.log(new NoteSequence(file).getSequence());			
		// });		
		// RequestMidi('http://localhost:8080/song/Castle_inSky.mid', function(data){
		// 	var file = MidiFile(data);
		// 	console.log(file);
		// 	console.log(new NoteSequence(file).getSequence());			
		// });
		// var address = 'http://localhost:8080/song/PiratesOfTheCaribbean.mid';
		var address = 'http://localhost:8080/song/Castle_inSky.mid';
		RequestMidi(address, function(data){
			var file = MidiFile(data);
			console.log(file);
			var seqObj = new NoteSequence(file).getSequence();
			console.log(seqObj);
			// console.log(seqObj.sequences[0][1]);
			var transObj = TransitionAnnalyse(seqObj.sequences[0][9], seqObj.ticksPerBeat);		
			console.log(transObj);

			var noteEvent = [];
			var r = CustomRandom($scope.seed);
			console.log($scope.seed);
			var current = {noteNumber : 'A3', beatLength : 16}

			var i = 0;
			var tick = 0;
// 			MIDI.noteOn(0, MIDI.keyToNote[current.noteNumber], 127, $scope.delay_time / 1000.0 * tick);
// 			tick += current.beatLength / 32;
// 			MIDI.noteOff(0, MIDI.keyToNote[current.noteNumber], 127, $scope.delay_time / 1000.0 * tick);
// 			for (var i = 0; i < 100; ++i)
// 			{
// 				var ary;
// 				for (var j in transObj.noteObj)
// 				{
// 					if (j.split(' ')[0] == current.noteNumber &&
// 						j.split(' ')[1] == current.beatLength)
// 					{
// 						ary = transObj.noteObj[j];
// 						break;
// 					}
// 				}
// 				var idx = Math.floor(r.next(0, ary.length));
// 				var toBe = ary[idx];
// 				current = toBe;
// 				// console.log(current);
// 				MIDI.noteOn(0, MIDI.keyToNote[current.noteNumber], 127, $scope.delay_time / 1000.0 * tick);
// 				tick += current.beatLength / 32;
// 				MIDI.noteOff(0, MIDI.keyToNote[current.noteNumber], 127, $scope.delay_time / 1000.0 * tick);
// //				console.log($scope.delay_time / 100.0 * tick);
// 			}
			MIDI.programChange(9, MIDI.GM.byName["synth_drum"].number);
			tick = 0;
			for (var i = 0; i < 100; ++i)
			{
				var note = Math.floor(r.next(0, 60));
				MIDI.noteOn(9, note, 127, $scope.delay_time / 1000.0 * tick);				
				tick += 0.1;
				MIDI.noteOff(9, note, 127, $scope.delay_time / 1000.0 * tick);
			}
		});				
	}
	...
	$scope.play = function () {
		...
		["C4", "E4", "G4"].forEach(function(note) {
			var n = MidiEvent.createNote(note);
		    ...
		    Array.prototype.push.apply(noteEvents, n);
		});
		...
		var midiString = 'data:audio/midi;base64,' + song.b64;


		console.log(MidiFile(window.atob(midiString.split(',')[1])));
		console.log(midiString);
		$scope.player.loadFile(midiString, $scope.player.start);
		$scope.player.start();
	}
